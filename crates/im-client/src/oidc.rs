//! im-client's OIDC half: the drop-in client for topcoat apps that let im
//! hold their sign-in. Compiled under the `oidc` feature — on by default.
//!
...
//!
//! The model: im holds the central session; this half holds the app's
//! side of it — an encrypted cookie holding the opaque session token,
//! introspected against im on every request. The routes register through
//! the grammar's inventory, so a binary that also speaks OIDC through its
//! own client crate must build this half OFF (`default-features = false`),
//! or the two route sets collide at router build.
