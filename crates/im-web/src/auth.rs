...
/// through and the agent it claims to be. The address is the family's shared
/// [`server::client_label`], with `unknown` folded back to none so the
/// sessions list keeps its dash.
fn session_meta(cx: &Cx) -> im_core::sessions::SessionMeta {
    let headers = topcoat::router::request::headers(cx);
    let label = server::client_label(cx);
    let ip = (label != "unknown").then_some(label);
    let agent = headers
        .get("user-agent")
